...
const DrawingArea = () => {
  ...
  const [stickyNotesShow, setStickyNotesShow] = useState(false)
  const [boxes, setBoxes] = useState([])
  const [reSizeBoxId, setReSizeBoxId] = useState(null)
...
if(reSizeBoxId!==null){
  const updatedBoxes=boxes.map((ele)=>{
    if(ele.id==reSizeBoxId){
        const newWidth=e.clientX-initalData.startX
        const newHeight=e.clientY-initalData.startY
        return{
          ...ele,
          width:Math.max(initalData.startWidth+newWidth,50),
          height:Math.max(initalData.startHeight+newHeight,50)
        }
    }
    return ele
  })
...
return()=>{
  ...
  const handleBoxClick = (event) => {
    const flag = event.target.tagName === "TEXTAREA" || event.target.closest("TEXTAREA")
    if (
      !flag&&
      !event.target.closest('.closeNotesButton')&&
      !event.target.closest('.resizeHandle')&&stickyNotesShow
    ) {
      ...
      setBoxes((prev)=>[...prev,newAddedBox])
      // setStickyNotesShow(false)
    }

  }

  const handleTextChange=(e,boxId)=>{
    const val=e.target.value
    setBoxes((prev)=>
    prev.map((ele)=>ele.id===boxId?{...ele,text:val}:ele))
  }

  const handleBoxDelete=(boxId)=>{
    setBoxes((prev)=>prev.filter((ele)=>ele.id!==boxId))
  }

const handleMouseResize=(e,boxId)=>{
setReSizeBoxId(boxId)
setInitialData({
  startX:e.clientX,
  startY:e.clientY,
  startWidth:boxes.find((ele)=>ele.id===boxId).width,
  startHeight:boxes.find((ele)=>ele.id===boxId).height,
})
    ...
    <>
      ...
      >
        ...
        <div style={{ display: "flex" }}>
            ...
            >
              ...
              >
                <div
                  ...
                  onClick={() => setStickyNotesShow(true)}
                  style={{ padding: "12px" }}
                >
                  <BsStickyFill />
                </div>
          ...
          {
            stickyNotesShow ?
              <div style={{position:"relative",height:"100vh",width:"100%",background:"white"}} onClick={handleBoxClick} >
              {
                  boxes.map((ele) => {
                    return (
                      <Box
                  ...
                  borderRadius={"10px"}
                      >
                        ...
                        <Button
                          className="closeNotesButton"
                          pos={"absolute"}
                          top={"5px"}
                          right={"5px"}
                          zIndex={100}
                          onClick={()=>handleBoxDelete(ele.id)}
                        >
                        <RxCross2 fontSize={"20px"} fontWeight={1000} />
                        </Button>
                        <Textarea
                          value={ele.text}
                          onChange={(event)=>handleTextChange(event,ele.id)}
                          resize="none"
                          width="100%"
                          height="100%"
                        ...
                        onMouseDown={(e)=>handleMouseResize(e,ele.id)}
                        >
